...
|---|---|
| `deploy` | Build + push + create/update service + post-deploy probe + auto-heal (probe failure is ATTRIBUTED before it is repaired — see Post-deploy probe attribution). Final summary derives status/replicas from current Swarm tasks (`?/N` or `?/global` when unavailable). Compose / Dockerfile / preset / nixpacks auto-detected. **Convergence verification runs on EVERY deploy** (was registry-only before 2026-09-22 — single-node deploys printed `Deployment complete` over a Swarm rollback). A Swarm rollback, a `paused`/`rollback_paused` update, or a spec whose `servel.deployment.id` is no longer this deploy's fails the deploy non-zero; the headline names the failed task's error and the deployment Swarm restored (`Swarm rolled this deploy back after task … failed: task: non-zero exit (1); the service runs deployment <id> again`). The label is re-checked right before the deploy is recorded live (after the probe); a failed deploy never moves the environment index. `deployment superseded` = another deploy of the same app won — not retried, check `servel audit --app <app>`. `--skip-convergence`/`--fast` skip the wait but not the final label check. Multi-service compose stacks are not covered yet. |
...
| Build node fills up repeatedly | Verify the timer exists: `servel ssh <server> -c 'systemctl list-timers servel-gc.timer --all'`. Absent → `servel remote provision <server>` installs it (or `servel server gc schedule --daily`). Inspect the policy: `servel ssh <server> -c 'cat /var/servel/scripts/servel-gc.sh'`; run it by hand with `bash /var/servel/scripts/servel-gc.sh` | Scheduled GC install used to be gated on the server being UNHEALTHY, so a healthy server was re-provisioned forever without ever getting `servel-gc.timer` — KN ran months with no timer and hit 94% (2026-08-11). Now reconciled on EVERY provision. The policy is a script (`/var/servel/scripts/servel-gc.sh`), NOT inline in the unit's ExecStart — two inline copies had drifted on backslash escaping and on whether release retention ran. GC prunes the **buildx** builder (the default builder is empty on a servel node), unused images **older than 7 days including tagged** (dangling-only missed 25.3GB), registry **tag** retention before blob GC (blob GC frees ~0 while blobs are tagged), and releases keeping 20/channel. Retention runs via `servel registry retain` (Go, talks to the registry API) — an older `registry-retain.sh` shell copy exec'd `wget`/`curl` INSIDE the registry container, which ships neither, and failed with `sh: curl: not found`. Measured on KN 2026-08-12: buildx 10.4GB + images 2.9GB + registry 54.9→26.0GB = node 82%→61%. **Since the disk-reclaim loop:** the timer is no longer the only control — the daemon reclaims cheap tier (tmp sweeps, buildx above the 5GB floor, stopped containers, DANGLING images, registry blob GC when the build queue is idle) whenever the disk is ≥90% used OR forecast days-to-full <3, capped at `disk_auto_reclaim_budget` (4) per `disk_auto_reclaim_budget_window` (24h) cluster-wide, with a breaker after 2 consecutive runs freeing <1GB (one critical alert, hint `servel df --growers`). **Object-storage registry live findings (2026-09-05, servel-official e2e):** `setup --storage` creates the bucket itself (`rclone mkdir`; the S3 driver never does — a missing bucket = healthy `/v2/` + `NoSuchBucket` on every push); `registry auto --force` deploys a registry on a SINGLE node; `deploy --force-registry` now really pushes on single node (EnsureRegistry used to return nil before checking the configured entry); read-only maintenance must be set as `REGISTRY_STORAGE_MAINTENANCE_READONLY={"enabled":true}` (the `_ENABLED=true` leaf form PANICS registry:2.8.3); migration measured: 26 s push pause for 29 MB, restart-bound. PRE-EXISTING TRAP: `-r X` on `remote registry <sub>` and other optional-positional commands ran on the DEFAULT remote until `f01a541` (`ResolveRemoteSimple` ignored the flag) — with older clients use the positional server name. FIXED on the same branch: operator `registry retain --keep N` (and the daily timer, which runs it) now keeps tags a live service pins with reason `in-use` and refuses to delete when the service list is unreadable — before, `--keep 1` deleted the running service's tag. Audit: `disk.auto_reclaim` / `disk.auto_reclaim_declined`. On the LEADER tagged images + tag retention STAY on the timer; on WORKERS (which never get the timer) the same reclaim also prunes unused images older than the shared 7-day window (`until=168h`, one constant with the GC script and `servel remote gc --all`). `servel remote gc schedule` also now ensures container log rotation in `/etc/docker/daemon.json` (json-file, max-size=50m, max-file=3) — previously only `remote provision` wrote it; it never restarts dockerd, so `systemctl restart docker` in a window is the operator's step, and existing containers keep old log opts until redeployed. |
...
**Activity indicator (stderr, TTY-only).** Every command that talks to a server keeps one live line on stderr naming the step in flight — `⠙ Inspecting services on KN... · 4s` — covering the connect and every remote command after it. It never touches stdout, and `SERVEL_NONINTERACTIVE=1` / `CI=1` / a non-terminal stderr suppress it entirely, so agent invocations see nothing. Do NOT merge it into stdout with `2>&1` when parsing `--json`: the frames are carriage-return overwrites and will corrupt the JSON. It also never prints the remote command itself, only the phrase for it — so it is not a source of command text for debugging.
...
- `--build-on <node>` -- Build on specific node. **Managers are excluded from auto-selection whenever a worker is eligible (2026-09-20)** — a build is the heaviest thing servel runs and the manager is the node whose loss takes the cluster with it. `--build-on` still wins (naming a host is intent), and it now WARNS when it is dropped instead of being silently discarded. **Builds now actually run on the selected node (2026-09-20):** a server-side deploy hops one SSH from the manager to the chosen worker, streams the context in, and builds there. Previously the selected node was printed and IGNORED — every server-side build ran wherever the deploy happened to be, i.e. the manager. A deploy that selected a remote host it genuinely cannot reach REFUSES and names `--local-build`, rather than quietly building on the wrong node. The hop needs `ssh -o BatchMode=yes <worker> true` to work from the manager, which `servel node add` already sets up. **Auto-selection also gates on disk before ranking on RAM:** a candidate at >=90% used (df semantics) is skipped with `disk gate skipped <host> at N% (>=90% used)`; an UNMEASURED node is neutral (never excluded); if every candidate is over the ceiling the least-full one is used anyway. The gate only re-routes — the >=95% preflight is still the only thing that BLOCKS a deploy (`--force-low-disk` overrides that, 85-94% only warns).
...
# The SAME df definition now also drives the daemon's local pressure report and the capacity snapshot
# (both read Bfree before, under-reporting ~6pts at the danger end).
# Disk ALERTING thresholds are 80 warn / 90 critical — plus a TREND arm: forecast days-to-full < 3 is
# critical whatever the percentage, the message carries "full in ~N days at current trend", and a
# warning->critical transition bypasses the 1h cooldown. Unknown forecast = no opinion (percentage
# decides), never urgency. The local pressure REPORT keeps its own 75/85 levels.
...
**Compose-sourced templates: `env_file` IS honoured, with two rules (fixed 2026-08-08).** Servel resolves each service's upstream `env_file:` and inlines it into the Swarm spec, so upstream vars are *present* — the failure mode is wrong values, never missing ones. Two defects made that silent until 2026-08-08:

- `${VAR}` inside a per-service env file used to resolve against an empty environment, so upstream lines like `AWS_ACCESS_KEY_ID="${COMMON_S3_KEY}"` became `""` and `AWS_ENDPOINT="${COMMON_PROTOCOL}://${COMMON_DOMAIN_NAME}"` became the literal `://`. They now resolve from the template's `env_file_path` + template vars. **Don't restate a var in `overrides.environment` just to inject a credential** — override only what upstream gets wrong for Swarm (k8s `*.svc.cluster.local` hostnames, S3 endpoints aimed at the public domain, Kafka).
- An override written as `KEY: ""` used to be dropped by a `!= ""` guard, so `KAFKA_SERVERS: ""` never applied and the k8s value survived. An explicit empty literal now blanks the value; a `{{ .Var }}` that *renders* empty still leaves the existing value alone (unset vars must not clobber generated credentials).

Live fallout on `noras-openreplay` before the fix, with all 23 services reporting 1/1: alerts crash-looping on `NameResolutionError ... clickhouse-openreplay-clickhouse.db.svc.cluster.local` (its `ch_host` was never overridden — it reads `ch_host`, not `CLICKHOUSE_STRING`), chalice signing refresh tokens with an empty HS512 key, http's `TOKEN_SECRET` empty while images/canvases held a real one, spot and sourcemapreader holding blank S3 credentials. Diagnose any compose-sourced infra with `docker service inspect <svc> --format '{{range .Spec.TaskTemplate.ContainerSpec.Env}}{{println .}}{{end}}'` and grep for `=$`, `=://`, and `cluster.local`.
...
**Common server keys**: `build_queue.{enabled,max_concurrent,queue_timeout,priority_deployments,per_host_concurrency}`, `log_retention.{max_age_days,max_size_mb,compress,schedule}`, `build_cache.{max_size_gb,max_age_days,prune_on_deploy,keep_storage_gb,registry_export,registry_export_mode}` (**`keep_storage_gb` default 5 is now ONE constant** shared by daemon.json `builder.gc.defaultKeepStorage`, the GC script, `servel prune --all`, the deploy low-disk sweep, `servel cache config --keep-size` and the daemon reclaim — it used to be four different numbers 20GB/5g/5/3g; `servel remote gc --keep-storage` still overrides per run), `registry_retention.{keep_per_repo,older_than,always_keep}`, `deployment_retention`, `port_range_start`, `port_range_end`, `access.{enabled,audit_retention_days}`. Run `servel config list --server --defaults` for the full inventory on any remote.
...
# Containment (daemon, default-on): crash-loop quarantine — a storm is one replica slot with >=5 restarts in 10 min (read from Swarm task history by the leader every 5 min); a service storming >=2 consecutive windows is scaled to 0 (budget 3/24h cluster-wide, 1/stack; stack root-folding quarantines only the -db root; >5 unrelated storms = node-level breaker, alert only; release: servel scale <name> 1 / servel infra repair <infra>). Infra dup-generation reconciler force-converges services running MORE tasks than desired (sustained 2 checks, budget 5/24h).
...
**Caller identity server-side = the gate user, never `auth login`.** The gate runs every user's commands as one server account and passes the authenticated user in `SERVEL_GATE_USER`. Server-side access commands resolve the caller with `auth.Principal()` / `auth.GatePrincipal()` (`internal/auth/principal.go`), NOT `auth.GetCurrentUser()`. The server account's `~/.servel/auth.json` and `SERVEL_USER` are self-declared, so any gated user could point them at someone else. Consequences:
- `access leave` removes the gate user's own record + key. `access request create`/`cancel` act as the gate user. All three REFUSE without the gate (direct root SSH has no access user): `cannot tell which access user is leaving: not invoked through the access gate (SERVEL_GATE_USER is not set)`.
- Self-approval is checked against the gate user. `reviewed_by` (approve/deny/modify/extend/revoke) and invite/token/user `created_by` record the gate user, or the server account (e.g. `root`) for a direct-SSH admin. Root admins still approve and invite.
- `servel auth login` never changes any of this. It is client-side attribution only.
- `servel audit list` `user` is the gate user for every entry written under the gate (`audit.Logger.Log` fills it from `auth.GatePrincipal` and overrides any caller-set `User`), including access approve/deny/invite; the subject stays in `metadata.target_user`. Without the gate it is the OS account (or a caller-set value such as `daemon`). Before this fix gated actions were audited as `root`.
- The gate refuses every leading `NAME=value` except `SERVEL_SERVER_MODE`, `SERVEL_OPERATOR_NAME`, `SERVEL_OPERATOR_EMAIL`, `SERVEL_EVENTS` (`setting X is not permitted through the access gate`). A command therefore cannot set its own `SERVEL_GATE_USER`, `SERVEL_USER`, `HOME`, `PATH`, `LD_PRELOAD` or `SERVEL_DATA_DIR`. Adding a new client-sent prefix means adding it to `clientEnvAssignments` in `internal/access/gate/gate.go`.
- New server-side access code must use `auth.Principal`. `TestAccessCommandsTakeIdentityFromPrincipal` (`internal/cli/access/access_principal_test.go`) fails `go test` if a command there calls `auth.GetCurrentUser`/`GetIdentityInfo`/`LoadAuthContext`.

**The gate runs exactly what it checked.** Team members' commands reach the server through the gate, and the client does not know it is gated, so every command a CLI path sends over SSH must be one the gate can classify.
- **`servel …` commands** are exec'd as `/usr/local/bin/servel` with the classified argv. No shell runs. Unquoted `; & | ( ) < >`, newlines, `$` and backticks (even inside double quotes) are refused with `servel commands run without a shell: ";" is not permitted…`. Build them with `shell.BuildCommand` / `executeRemoteServel` (package cli). Never append `2>/dev/null`, `2>&1`, `|| true`, `|| echo '{}'`, `| jq` or `$(hostname)`: handle stderr and failure client-side. The binary must be `servel` or `/usr/local/bin/servel`.
- **System commands** (`docker …`, `cat …`) still run via `sh -c`, and every segment, pipe stage and redirection is classified. Any part it cannot classify refuses the whole string (`unknown or unclassifiable command`).
  - Refused: `for`/`while`/`if` scripts, `$VAR` (only `$HOME` passes), `xargs … sh -c`, `find -exec sh -c`, `curl | sh`, binaries named by a path outside the system bin dirs, and docker actions the gate does not list.
  - Use instead: `find … -printf 'FILE:%p\n' -exec cat {} \;` rather than `-exec sh -c 'echo $1…'`, `xargs -r docker <verb> <action>` rather than `while read`, and `docker.ServiceTasksCmd(names, args...)` for per-service `docker service ps`.
- A team member reporting one of those errors means the client command needs reshaping. Fix the builder, never loosen the gate. Pin it with a test that runs the sent command through `gate.ParseCommand` (see `internal/cli/state_validation_test.go`, `internal/swarm/gate_compatible_test.go`).
- Raw commands are hardened, not airtight: through them `config-write`, `update` and `deploy` still reach root on the node. Rules and path tiers: website `reference/security.mdx`.
...
# Deploy-time retention (automatic, every deploy) now enumerates tags from the REGISTRY API, not the
# local image list — a tag used to stop being a deletion candidate once the daily image prune removed
# the local copy, so registries grew forever while the pass reported success. Deletes bare v<N> tags
# outside the deployment_retention window (default 3); never latest/stable/main/master/buildcache, never a tag a live
# service references, never a digest a kept tag shares. NOTE the window is server `deployment_retention`
# (default 3), NOT `registry_retention.keep_per_repo` (default 10) — that one drives the SCHEDULED
# `servel registry retain` / daily timer. Every SKIP is a deploy event + a
# `registry.retention` audit entry (reasons: no-registry, no-credentials, keep-all, unreachable,
# timeout, deletion-unsupported, kept-digest-unknown, live-images-unknown, repo-owners-unknown,
# shared-registry-path). It can never fail a deploy.
# `shared-registry-path`: two deployments from ONE git repo push bare v<N> tags into the same
# owner/repo, and keep-N is PER DEPLOYMENT — applying it to the union silently prunes the quieter one
# down to its live pin. servel names it and prunes nothing; fix with `deploy.registry_path` in
# servel.yaml for one of them.
...
**Always-preserved tags** for retain: `latest`, `stable`, `main`, `master`, plus any tag a live service spec pins (`in-use`, does not consume a `--keep` slot). Daily systemd timer runs retention + GC at `/var/servel/scripts/registry-retain.sh`.
...
| Deploy failed with `Swarm rolled this deploy back after task … failed` | The new container crashed within Swarm's update-monitor window (usually on start — missing env/secret, bad config) and Swarm restored the previous spec. The previous deployment is still serving and still current; nothing to clean up. `servel logs <app> --previous` for the crash output (until the failed-task log capture fix ships it may show only the exit status; the full deploy error is in `servel logs <app> --op 1`); fix and redeploy. |
| `superseded` status in `servel ps` / multiple generations of one app | **Normal, not an error.** Each deploy is an immutable generation; Servel keeps the last `deployment_retention` (default **3**) and marks superseded (old) generations `superseded`. Exactly one generation per app is `running`. A leader-side daemon reconcile pass (`generation_reconcile_budget`, default 50) collapses historical duplicates. The live generation's dir is never GC'd, even after `rollback`. "Live" = the index's current generation PLUS every generation the app's Swarm service depends on via `servel.deployment.id`: service spec label, `PreviousSpec` label (rollback target), and each running task's container label — they diverge after a Swarm rollback or a label-only spec rewrite (navola 2026-09-21: pruning the label's dir broke `env set`/`secrets` resolution). Unreadable label → deploy skips pruning with `Warning: skipping old-version cleanup: …`; `servel migrate --cleanup` refuses outright. |
